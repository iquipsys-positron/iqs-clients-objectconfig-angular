export class AddEqupmentDialogController implements ng.IController {
    public $onInit() { }
    public theme;
    ...
    public config() {
        if (this.$state.current.name != 'settings_system.objects') {
            ...
            window.location.href = window.location.origin + `/settings_system/index.html#/settings_system/objects`;
            this.$mdDialog.hide();
        }
        this.$mdDialog.hide();
    }
